// using-functions
flash_dropdown_menu();
flash_dropdown_mobile_menu(false);
flash_search_form();
flash_modal_user_form_warnings();
flash_mobile_menu(false);
...
function flash_dropdown_mobile_menu(item_icon = true, hide_other_items = true) {
	...
	function active_toggler(btn) {

		for (var i = 0; i < toggler.length; i++) {
			
			if (toggler[i] == btn) toggler[i].classList.toggle('active');
			else toggler[i].classList.remove('active');
		}
	}

	function hide_others(item) {

		if (!hide_other_items) return;

		for (var i = 0, items = []; i < toggler.length; i++) {

			var target = 

			items[i] = document.getElementById(''+toggler[i].getAttribute('target')+'');

			if (items[i] == item) continue;

			else flash_remove_classes(items[i], ["show","fade-in","slide-down"]);
	...
	function toggle_icon(icon) {
		if (icon != undefined && item_icon) icon.classList.toggle('fa-angle-down');
	}
...
// mobile-menu
function flash_mobile_menu(icon = true) {
...
function flash_fixed_header() {
	...
	// adding-functions
	window.addEventListener('scroll', make_fixed);

	// functions
	function make_fixed() {

		if (document.documentElement.scrollTop > 16) header.classList.add('flash-fixed-header');
		else header.classList.remove('flash-fixed-header');
	}
